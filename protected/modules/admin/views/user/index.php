                ...
                <div class="widget-body">
                    <div class="widget-main">
                        <form class="form-inline" method="get">
                            <input type="text" name="name" value="<?php echo Yii::app()->request->getParam('name', '') ?>" class="input-medium" placeholder="姓名">
                            <select name='depart_id'>
                                <option value='0'>--请选择部门--</option>
                                <?php foreach (Depart::getDepart() as $key => $val): ?>
                                    <option <?php if ($key == Yii::app()->request->getParam("depart_id",0) ): ?>selected="selected"<?php endif ?> value='<?php echo $key ?>'><?php echo $val ?></option>
                                <?php endforeach ?>
                            </select>
                    ...
                    <tbody>
                        <?php foreach ($model as $k => $v): ?>
                            <tr>
                                ...
                                <td><?php echo $v->depart->name ?></td>
                                <td><?php echo date('Y-m-d H:i:s', $v->created) ?></td>
                                <td>
                                    <a href="<?php echo $this->createUrl('placeManage', array('id' => $v->id)) ?>">采价点管理</a>
